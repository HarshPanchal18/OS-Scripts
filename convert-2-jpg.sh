...
#------------------------------------------
# Purpose: A bash script to convert FROM gif, bmp, pdf, webp, pcx, pict, pct, png, tif, tiff, xcf, crw, cr2, arw, sr2 & nef TO jpg/jpeg formats.

# Created Date:  Saturday 04 June 2022 03:54:06 PM IST

# Author: Harsh Panchal

#Dependencies: gimp, imagemagick, dcraw

# Usage:
# cd photos_directory
# ./convert-2-jpg.sh [file(s)_to_convert]
#------------------------------------------

# ---------------check if an image exists, else exit----------
if [ "`ls | grep -i
    '\(\.jpg\|\.JPG\|\.jpeg\|\.JPEG\|\.gif\|\.GIF\|\.bmp\|\.BMP\|\.pdf\|\.PDF\|\.webp\|\.WEBP\|\.pcx\|\.PCX\|\.pict\|\.PICT\|\.pct\|\.PCT\|\.png\|\.PNG\|\.tif\|\.TIF\|\.tiff\|\.TIFF\|\.xcf\|\.XCF\|\.crw\|\.CRW\|\.cr2\|\.CR2\|\.arw\|\.ARW\|\.sr2\|\.SR2\|\.nef\|\.NEF\)'    | wc -l`" -eq 0 ]
then
    echo ""
    echo "No Image File exists in current directory"
    echo ""
    exit
else
...
then
    ...
    mkdir ./originals
    cp *.* ./originals
    echo "Directory original created, copying photos [ok]"
fi
...
then
    for i in *.jpeg
    do
        mv "$i" "`basename $i .jpeg`-jpeg.jpeg"
    done
    mv *.jpg \jpg
fi
echo "Moving *.jpeg [ok]"
